...
const Post = () => {
    ...
    const { mypost, setEditPost, post, user } = useAuthContext()
    ...
    const handleDeleteArticle = () => {
        ...
        axiosClient.delete(`/delete/article/${post.id}`)
            .then((response) => {
                console.log('dele:', response.data.message )
    ...
    return (
        ...
        <div className='w-full relative overflow-y-aut px-10'>
           <div className='absolute text-center bg-red-500 text-lg text-white rounded w-8/12 h-9'>{deleteMessage}</div> 
            ...
            <div className=' w-10/12'>
                <div className='bg-green-500 hover:bg-green-600 cursor-pointer hover:text-neutral-300 text-xs rounded p-1  inline-block'>
                    <i className='text-sm'>Created on:</i> {postCreated} | <span>Category:{post.category}</span> | <i className='underline '>Author:</i>  </div>
            </div>
            <div className=' max-w text-contain text-container mb-12'>
