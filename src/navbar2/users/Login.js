import React from 'react'
import { Link } from 'react-router-dom'

const Login = () => {
  return (
    <div className='w-full grid place-items-center h-screen' style={{backgroundImage:`url("data:image/jpeg;base64,")`}}>
    <form className='w-4/12 bg-slate-transparent border p-2 h-[20rem]' >
    <p className='text-center text-2xl text-neutral-300 pt-4'>Login Here</p>
    
    <div className='p-2 mt-4'>
    <label className='text-neutral-300 font-bold'>Email</label><br/>
    <input type="text" name='email' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>
    <div className='p-2 mt-4'>
    <label className='text-neutral-300 font-bold'>Password</label><br/>
    <input type="text" name='name' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>
   

    <div className='w-full flex pl-8 justify-between mt-4'>
    <button className='bg-green-400 w-6/12 rounded font-bold text-neutral-300 hover:bg-green-500 h-8 '>Login</button>
    <Link className="text-xs text-neutral-300 underline" to="/register">Not Register? Register</Link>
    </div>

    </form>
    </div>
  )
}

export default Login
