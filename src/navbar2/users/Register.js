import React from 'react'
import image from '../images/tech.gif'
import { Link } from 'react-router-dom'

const Register = () => {
  return (
    <div className="w-full grid place-items-center bg-transparent pb-6 h-screen" style={{backgroundImage:`url("data:image/jpeg;base64,")`}}>
    <form className='w-4/12 bg-transparent border p-2 ' >
    <p className='text-center text-2xl text-neutral-300'>Register Here</p>
    <div className='p-2'>
    <label className='text-neutral-300 font-bold'>Name</label><br/>
    <input type="text" name='name' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>
    <div className='p-2'>
    <label className='text-neutral-300 font-bold'>Email</label><br/>
    <input type="text" name='name' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>
    <div className='p-2'>
    <label className='text-neutral-300 font-bold'>Password</label><br/>
    <input type="text" name='name' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>
    <div className='p-2'>
    <label className='text-neutral-300 font-bold'>Confirm Password</label><br/>
    <input type="text" name='name' className='w-8/12 text-neutral-300 h-8 bg-transparent border rounded' />
    </div>

    <div className='w-full flex pl-8 justify-between'>
    <button className='bg-green-400 w-6/12 rounded font-bold text-neutral-300 hover:bg-green-500 h-8 '>Register</button>
    <Link className="text-xs text-neutral-300 underline" to="/login">Already Register? Login</Link>
    </div>

    </form>
    </div>
  )
}

export default Register